...
8010373a:	68 64 76 10 80       	push   $0x80107664
...
80103a54:	31 f6                	xor    %esi,%esi
    ...
    //RR
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80103a56:	bf 54 2d 11 80       	mov    $0x80112d54,%edi
  for(;;){
    // Enable interrupts on this processor.
    sti();
    ...
    // Loop over process table looking for process to run.
    acquire(&ptable.lock);
80103a5b:	68 20 2d 11 80       	push   $0x80112d20
...
80103a6a:	8d b6 00 00 00 00    	lea    0x0(%esi),%esi
    ...
    //RR
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80103a70:	83 c7 7c             	add    $0x7c,%edi
80103a73:	81 ff 54 4c 11 80    	cmp    $0x80114c54,%edi
80103a79:	74 65                	je     80103ae0 <scheduler+0xb0>
      cprintf("problem 1\n");
80103a7b:	83 ec 0c             	sub    $0xc,%esp
80103a7e:	68 b0 75 10 80       	push   $0x801075b0
80103a83:	e8 d8 cb ff ff       	call   80100660 <cprintf>
		if(p->state != RUNNABLE) continue;
80103a88:	83 c4 10             	add    $0x10,%esp
80103a8b:	83 7f 0c 03          	cmpl   $0x3,0xc(%edi)
80103a8f:	75 df                	jne    80103a70 <scheduler+0x40>
      if( p->pid % 2 == 0 )
80103a91:	f6 47 10 01          	testb  $0x1,0x10(%edi)
80103a95:	75 d9                	jne    80103a70 <scheduler+0x40>
      {
        flag = 1;
        c->proc = p;
        switchuvm(p);
80103a97:	83 ec 0c             	sub    $0xc,%esp
      ...
      if( p->pid % 2 == 0 )
      {
        flag = 1;
        c->proc = p;
80103a9a:	89 bb ac 00 00 00    	mov    %edi,0xac(%ebx)
        switchuvm(p);
80103aa0:	57                   	push   %edi
    acquire(&ptable.lock);
    ...
    //RR
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80103aa1:	83 c7 7c             	add    $0x7c,%edi
		if(p->state != RUNNABLE) continue;
      if( p->pid % 2 == 0 )
      {
        flag = 1;
        c->proc = p;
        switchuvm(p);
80103aa4:	e8 b7 2e 00 00       	call   80106960 <switchuvm>
        p->state = RUNNING;
80103aa9:	c7 47 90 04 00 00 00 	movl   $0x4,-0x70(%edi)
        swtch(&(c->scheduler), p->context);
80103ab0:	59                   	pop    %ecx
80103ab1:	5e                   	pop    %esi
80103ab2:	ff 77 a0             	pushl  -0x60(%edi)
80103ab5:	ff 75 e4             	pushl  -0x1c(%ebp)
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      cprintf("problem 1\n");
		if(p->state != RUNNABLE) continue;
      if( p->pid % 2 == 0 )
...
80103ab8:	be 01 00 00 00       	mov    $0x1,%esi
        ...
        p->state = RUNNING;
        swtch(&(c->scheduler), p->context);
80103abd:	e8 39 0c 00 00       	call   801046fb <swtch>
        switchkvm();
80103ac2:	e8 79 2e 00 00       	call   80106940 <switchkvm>
        c->proc = 0;
80103ac7:	83 c4 10             	add    $0x10,%esp
    ...
    //RR
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80103aca:	81 ff 54 4c 11 80    	cmp    $0x80114c54,%edi
        c->proc = p;
        switchuvm(p);
        p->state = RUNNING;
        swtch(&(c->scheduler), p->context);
        switchkvm();
        c->proc = 0;
80103ad0:	c7 83 ac 00 00 00 00 	movl   $0x0,0xac(%ebx)
80103ad7:	00 00 00 
    acquire(&ptable.lock);
    ...
    //RR
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80103ada:	75 9f                	jne    80103a7b <scheduler+0x4b>
...
80103ae2:	75 76                	jne    80103b5a <scheduler+0x12a>
80103ae4:	bf 54 2d 11 80       	mov    $0x80112d54,%edi
80103ae9:	89 f8                	mov    %edi,%eax
80103aeb:	eb 0d                	jmp    80103afa <scheduler+0xca>
80103aed:	8d 76 00             	lea    0x0(%esi),%esi
    {
      struct proc *p_tmp = ptable.proc;
      for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80103af0:	83 c0 7c             	add    $0x7c,%eax
80103af3:	3d 54 4c 11 80       	cmp    $0x80114c54,%eax
80103af8:	74 2b                	je     80103b25 <scheduler+0xf5>
        if(p->state != RUNNABLE) continue;
80103afa:	83 78 0c 03          	cmpl   $0x3,0xc(%eax)
80103afe:	75 f0                	jne    80103af0 <scheduler+0xc0>
        //홀수이면서 제일 pid가 작은 것을 찾아준다.
        if( p->pid % 2 == 1 && (p_tmp->pid > p->pid)) p_tmp=p;
80103b00:	8b 48 10             	mov    0x10(%eax),%ecx
80103b03:	89 ce                	mov    %ecx,%esi
80103b05:	c1 ee 1f             	shr    $0x1f,%esi
80103b08:	8d 14 31             	lea    (%ecx,%esi,1),%edx
80103b0b:	83 e2 01             	and    $0x1,%edx
80103b0e:	29 f2                	sub    %esi,%edx
80103b10:	83 fa 01             	cmp    $0x1,%edx
80103b13:	75 db                	jne    80103af0 <scheduler+0xc0>
80103b15:	3b 4f 10             	cmp    0x10(%edi),%ecx
80103b18:	0f 4c f8             	cmovl  %eax,%edi
    ...
    if(flag == 0)
    {
      struct proc *p_tmp = ptable.proc;
      for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
80103b1b:	83 c0 7c             	add    $0x7c,%eax
80103b1e:	3d 54 4c 11 80       	cmp    $0x80114c54,%eax
80103b23:	75 d5                	jne    80103afa <scheduler+0xca>
        if(p->state != RUNNABLE) continue;
        //홀수이면서 제일 pid가 작은 것을 찾아준다.
        if( p->pid % 2 == 1 && (p_tmp->pid > p->pid)) p_tmp=p;
      }
...
80103b25:	83 ec 0c             	sub    $0xc,%esp
      ...
      c->proc = p_tmp;
80103b28:	89 bb ac 00 00 00    	mov    %edi,0xac(%ebx)
      switchuvm(p_tmp);
80103b2e:	57                   	push   %edi
80103b2f:	e8 2c 2e 00 00       	call   80106960 <switchuvm>
      p_tmp->state = RUNNING;
80103b34:	c7 47 0c 04 00 00 00 	movl   $0x4,0xc(%edi)
      swtch(&(c->scheduler), p_tmp->context);
80103b3b:	58                   	pop    %eax
80103b3c:	5a                   	pop    %edx
80103b3d:	ff 77 1c             	pushl  0x1c(%edi)
80103b40:	ff 75 e4             	pushl  -0x1c(%ebp)
80103b43:	e8 b3 0b 00 00       	call   801046fb <swtch>
      switchkvm();
80103b48:	e8 f3 2d 00 00       	call   80106940 <switchkvm>
      c->proc = 0;
80103b4d:	c7 83 ac 00 00 00 00 	movl   $0x0,0xac(%ebx)
80103b54:	00 00 00 
...
80103bf0:	68 bb 75 10 80       	push   $0x801075bb
...
80103bfd:	68 e7 75 10 80       	push   $0x801075e7
...
80103c0a:	68 d9 75 10 80       	push   $0x801075d9
...
80103c17:	68 cd 75 10 80       	push   $0x801075cd
...
80103d3d:	68 08 76 10 80       	push   $0x80107608
...
80103d4a:	68 fb 75 10 80       	push   $0x801075fb
...
80103e59:	68 1a 76 10 80       	push   $0x8010761a
...
80103e66:	68 14 76 10 80       	push   $0x80107614
...
8010408e:	83 f8 05             	cmp    $0x5,%eax
      ...
      state = "???";
80104091:	ba 2b 76 10 80       	mov    $0x8010762b,%edx
  uint pc[10];
...
80104098:	8b 14 85 8c 76 10 80 	mov    -0x7fef8974(,%eax,4),%edx
      state = states[p->state];
    else
      state = "???";
8010409f:	b8 2b 76 10 80       	mov    $0x8010762b,%eax
...
801040ae:	68 2f 76 10 80       	push   $0x8010762f
...
8010411a:	68 a4 76 10 80       	push   $0x801076a4
...
80104355:	68 c6 76 10 80       	push   $0x801076c6
...
80104362:	68 af 76 10 80       	push   $0x801076af
...
80104443:	68 cd 76 10 80       	push   $0x801076cd
...
8010448c:	68 d5 76 10 80       	push   $0x801076d5
...
801048e8:	68 fb 76 10 80       	push   $0x801076fb
...
80104913:	68 dd 76 10 80       	push   $0x801076dd
